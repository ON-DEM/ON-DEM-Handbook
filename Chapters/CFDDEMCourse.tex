\chapter[Open-source CFD--DEM with preCICE]{An Introduction to Open-Source CFD--DEM Simulations with preCICE}
...
The full material for the course including code and required input files can be obtained from \url{10.5281/zenodo.22706923}.
...
export PRECICE_INSTALL_DIR="$COURSE_ROOT/software/precice-install"
...
cmake \
  ...
  -DCMAKE_INSTALL_PREFIX="$PRECICE_INSTALL_DIR" \
...
\noindent\begin{minipage}{\linewidth}
\begin{CourseCode}
export CMAKE_PREFIX_PATH="$CMAKE_PREFIX_PATH:$PRECICE_INSTALL_DIR"
export PKG_CONFIG_PATH="$PKG_CONFIG_PATH:$PRECICE_INSTALL_DIR/lib/pkgconfig"
export LD_LIBRARY_PATH="$LD_LIBRARY_PATH:$PRECICE_INSTALL_DIR/lib"
\end{CourseCode}
\end{minipage}
...
cmake \
  -DCMAKE_BUILD_TYPE=Release \
  -DCMAKE_PREFIX_PATH="$PRECICE_INSTALL_DIR" \
...
\nolinkurl{PRECICE_INSTALL_DIR}, \nolinkurl{MERCURYDPM_BUILD_DIR}, and
the preCICE search paths configured in case 1. Make sure that persistent
...
mkdir -p build
cd build
export VORO_INSTALL_DIR="$COURSE_ROOT/software/voro-install"
cmake \
  -DCMAKE_BUILD_TYPE=Release \
  -DCMAKE_POSITION_INDEPENDENT_CODE=ON \
  -DCMAKE_INSTALL_PREFIX="$VORO_INSTALL_DIR" \
  ..
make -j4
make install
export CMAKE_PREFIX_PATH="$CMAKE_PREFIX_PATH:$VORO_INSTALL_DIR"
\end{CourseCode}
\end{minipage}
...
mkdir -p build
cd build
export LIGGGHTS_INSTALL_DIR="$COURSE_ROOT/software/liggghts-install"
cmake \
  -DCMAKE_INSTALL_PREFIX="$LIGGGHTS_INSTALL_DIR" \
  -DCMAKE_BUILD_TYPE=Release \
  -DENABLE_PRECICE=ON \
...
make -j4
make install
export PATH="$LIGGGHTS_INSTALL_DIR/bin:$PATH"
\end{CourseCode}
\end{minipage}

As in case 1, the optional Voro++ and LIGGGHTS exports must be set in
every terminal that builds or runs LIGGGHTS. Add
\nolinkurl{VORO_INSTALL_DIR}, \nolinkurl{LIGGGHTS_INSTALL_DIR}, the
updated \nolinkurl{CMAKE_PREFIX_PATH}, and \nolinkurl{PATH} commands to
\nolinkurl{~/.bashrc} if you want to keep them across terminals.
